...
class ContractService {
    ...
    async createContract(contractData) {
        ...
        // V2: gera codigo ELO-YYYY-XXXXXX + campos PT + mapeia status
        const codigo = contractData.codigo || `ELO-${new Date().getFullYear()}-${this.generateId().toUpperCase().slice(-6)}`;
        const periodos = contractData.periodos || (contractData.schedule ? CONFIG.scheduleToPeriodos(contractData.schedule.start, contractData.schedule.end) : []);
        const newContract = {
            id: this.generateId(),
            codigo,
            titulo: contractData.titulo || `Cuidado - ${contractData.activities?.[0]||'assistência'}`,
            descricaoNecessidade: contractData.descricaoNecessidade || contractData.observations || contractData.activities?.join(', ') || '',
            enderecoAtendimento: contractData.enderecoAtendimento || contractData.clientId || '',
            ...contractData,
            valorHora: contractData.valorHora || contractData.hourlyRate,
            hourlyRate: contractData.hourlyRate || contractData.valorHora,
            valorTotal: contractData.valorTotal || null,
            periodos,
            status: CONFIG.CONTRACT_STATUS.DRAFT,
            statusV2: 'rascunho', // V2
            ...
            signedAt: null,
            totalValue: this.calculateContractValue({...contractData, hourlyRate: contractData.hourlyRate||contractData.valorHora}),
            documentUrl: null
        };
        newContract.valorTotal = newContract.totalValue.total;
     ...
     */
     async enrichContract(contract) {
        ...
        // Compat V2: api pode retornar {clienteId, profissionalId, codigo, valorTotal} — normaliza
        const normId = contract.clienteId || contract.clientId;
        const normProf = contract.profissionalId || contract.caregiverId;
        const hourly = contract.valorHora || contract.hourlyRate;
        
        return {
            ...contract,
            clientId: normId, clienteId: normId,
            caregiverId: normProf, profissionalId: normProf,
            hourlyRate: hourly, valorHora: hourly,
            client: clients.find(c => c.id === normId) || null,
            caregiver: caregivers.find(c => c.id === normProf) || null,
            valueBreakdown: this.calculateContractValue({...contract, hourlyRate: hourly}),
            statusLabel: this.getStatusLabel(contract.statusV2 || contract.status),
            canSign: this.canUserSign(contract),
            nextStep: this.getNextStep(contract)
        };
    }
    ...
    getStatusLabel(status) {
        const labels = {
            ...
            // V2 PT
            rascunho: "Rascunho", proposta: "Proposta", negociacao: "Em negociação",
            aguard_assinatura: "Aguardando assinatura", ativo: "Ativo", concluido: "Concluído",
            rescindido: "Rescindido", cancelado: "Cancelado", em_disputa: "Em disputa"
        };
        // normaliza V1->V2 para exibição futura
        const norm = String(status||'').toLowerCase();
        return labels[status] || labels[norm] || status;
    }
